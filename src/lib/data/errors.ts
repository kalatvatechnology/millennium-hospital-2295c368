export type DataErrorKind =
  "permission" | "authentication" | "invalid_value" | "unavailable" | "query";
...
export function classifyDataError(error: unknown): DataAccessError {
  ...
  if (normalized.includes("does not exist") || normalized.includes("schema cache")) {
    return new DataAccessError(
      "unavailable",
      "This feature is not available on the connected backend.",
      error,
    );
  }
  return new DataAccessError(
    "query",
    "We couldn't load or save this information. Please try again.",
    error,
  );
}
