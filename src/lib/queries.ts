...
export type { BlogPost, Department, Doctor as DoctorRow, Doctor as DoctorWithDepartment, Facility as FacilityRow, Faq as FaqRow, FaqCategory, MediaItem, Review as ReviewRow, Service as HospitalService, Service as ProfessionalService } from "@/lib/data/models";

export const departmentsQuery = queryOptions({ queryKey: ["departments"], queryFn: listDepartments });
export const departmentQuery = (slug: string) => queryOptions({ queryKey: ["department", slug], queryFn: () => getDepartment(slug) });
...
export const blogPostQuery = (slug: string) => queryOptions({ queryKey: ["blog-post", slug], queryFn: () => getBlogPost(slug) });
export const enquiryOptionsQuery = queryOptions({ queryKey: ["enquiry-options"], queryFn: getEnquiryOptions });
