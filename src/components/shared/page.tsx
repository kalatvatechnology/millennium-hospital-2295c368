...
export function ContentSection({ children, muted = false }: { children: ReactNode; muted?: boolean }) {
  return <section className={muted ? "border-y border-border/70 bg-secondary/55" : "bg-background"}><div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">{children}</div></section>;
}
...
export function SectionHeading({ eyebrow, title, description, link }: { eyebrow: string; title: string; description: string; link?: { label: string; to: "/about" | "/blog" | "/contact" | "/departments" | "/doctors" | "/facilities" | "/faq" | "/media" | "/reviews" | "/services" } }) {
  return <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div className="max-w-2xl"><p className="text-sm font-bold text-primary">{eyebrow}</p><h2 className="mt-3 text-3xl font-semibold sm:text-4xl">{title}</h2><p className="mt-4 leading-7 text-muted-foreground">{description}</p></div>{link ? <Link to={link.to} className="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-md px-2 text-sm font-semibold text-primary hover:bg-secondary">{link.label}<ArrowRight className="size-4" /></Link> : null}</div>;
}
