  ...
  return (
    <div className="min-h-screen bg-admin">
      <header className="sticky top-0 z-40 border-b border-border bg-background/95 shadow-[var(--shadow-sm)] backdrop-blur">
        <div className="mx-auto flex h-18 max-w-screen-2xl items-center gap-3 px-4 sm:px-6 lg:px-8">
          <Link to="/" className="flex items-center gap-3 font-heading font-semibold">
            <span className="grid size-9 place-items-center rounded-lg bg-primary text-primary-foreground"><Plus className="size-5" /></span>
            <span>The Millennium Hospital</span>
          </Link>
          <span className="hidden rounded-sm bg-secondary px-2 py-1 text-xs font-semibold text-primary sm:inline">Staff workspace</span>
          <p className="hidden text-sm text-muted-foreground md:block">{roles.map((role) => ROLE_LABELS[role]).join(", ")}</p>
          <Button variant="ghost" size="icon" className="ml-auto lg:hidden" onClick={() => setNavOpen((value) => !value)} aria-label={navOpen ? "Close staff navigation" : "Open staff navigation"} aria-expanded={navOpen}>{navOpen ? <X /> : <Menu />}</Button>
      ...
      <div className="mx-auto flex max-w-screen-2xl flex-col gap-8 px-4 py-6 sm:px-6 lg:flex-row lg:gap-10 lg:px-8 lg:py-8">
        <nav aria-label="Staff sections" className={`${navOpen ? "block" : "hidden"} rounded-lg bg-sidebar p-3 text-sidebar-foreground shadow-[var(--shadow-md)] lg:block lg:w-64 lg:shrink-0`}>
          <p className="px-3 pb-2 pt-1 text-xs font-semibold uppercase text-sidebar-foreground/55">Workspace</p>
              ...
              <li key={item.to}>
                <Link
                  to={item.to}
                   ...
                   activeProps={{ className: "block min-h-10 rounded-md px-3 py-2.5 text-sm font-semibold bg-sidebar-primary text-sidebar-primary-foreground" }}
                >
                  {item.label}
                </Link>
              </li>
