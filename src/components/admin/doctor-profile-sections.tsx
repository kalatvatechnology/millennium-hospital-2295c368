...
function SectionEditor({ doctorId, section }: { doctorId: string; section: Section }) {
  ...
  const add = async () => {
    ...
    if (!primaryField || !draft[primaryField.name]?.trim()) return setError(`${primaryField?.label ?? "First field"} is required.`);
