...
export function DoctorProfileSections({ doctorId }: { doctorId: string }) {
  return (
    <div className="grid gap-8">
      <VisibilityEditor doctorId={doctorId} />
      {sections.map((section) => (
        <SectionEditor key={section.table} doctorId={doctorId} section={section} />
      ))}
      <RelationshipEditor doctorId={doctorId} />
    </div>
  );
}
...
function SectionEditor({ doctorId, section }: { doctorId: string; section: Section }) {
  ...
  const add = async () => {
    ...
    if (!primaryField || !draft[primaryField.name]?.trim())
      return setError(`${primaryField?.label ?? "First field"} is required.`);
    ...
    section.fields.forEach((field) => {
      ...
      if (value) payload[field.name] = field.name.includes("year") ? Number(value) : value;
    });
  ...
  const remove = async (id: string) => {
    const { error: deleteError } = await db.from(section.table).delete().eq("id", id);
    if (deleteError) setError(userFacingDataError(deleteError));
    else void client.invalidateQueries({ queryKey: key });
  };
  const primaryName = section.fields[0]?.name ?? "id";
  return (
    <section className="rounded-lg border border-border p-4">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {section.fields.map((field) => (
          <div key={field.name} className={field.multiline ? "sm:col-span-2" : ""}>
            <Label htmlFor={`${section.table}-${field.name}`}>{field.label}</Label>
            {field.multiline ? (
              <Textarea
                id={`${section.table}-${field.name}`}
                value={draft[field.name] ?? ""}
                onChange={(event) =>
                  setDraft((old) => ({ ...old, [field.name]: event.target.value }))
                }
                className="mt-1"
              />
            ) : (
      ...
      <ul className="mt-4 grid gap-2">
        {query.data?.map((row: any) => (
          <li
            key={row.id}
            className="flex items-center justify-between gap-3 rounded-md bg-secondary p-3 text-sm"
          >
            ...
            <Button
              ...
              size="icon"
              variant="ghost"
              aria-label={`Remove ${row[primaryName]}`}
...
function VisibilityEditor({ doctorId }: { doctorId: string }) {
  ...
  const update = async (name: string, checked: boolean) => {
    const next = { ...query.data, [name]: checked };
    const { error } = await db
      .from("doctors")
      .update({ section_visibility: next })
      .eq("id", doctorId);
    if (!error) client.setQueryData(key, next);
  };
  return (
    <section className="rounded-lg border border-border p-4">
      <h3 className="font-semibold">Section visibility</h3>
      <p className="mt-2 text-sm text-muted-foreground">
        A section still stays hidden when it has no published content.
      </p>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {Object.entries(visibilityLabels).map(([name, label]) => (
          ...
          >
            <Checkbox
              checked={query.data?.[name] !== false}
              onCheckedChange={(value) => update(name, value === true)}
            />
            {label}
          </label>
        ))}
...
function RelationshipEditor({ doctorId }: { doctorId: string }) {
  return (
    <section className="rounded-lg border border-border p-4">
      <h3 className="font-semibold">Linked content</h3>
      <p className="mt-2 text-sm text-muted-foreground">
        Select existing CMS records to show on this doctor profile. Unpublished records remain
        hidden publicly.
      </p>
      <div className="mt-5 grid gap-6">
        {relationships.map((relation) => (
          <RelationshipGroup key={relation.key} doctorId={doctorId} relation={relation} />
        ))}
      </div>
    </section>
  );
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const [source, links] = await Promise.all([
        ...
        db.from(relation.link).select(relation.sourceId).eq("doctor_id", doctorId),
      ]);
      if (source.error) throw source.error;
      if (links.error) throw links.error;
      return {
        options: source.data ?? [],
        linked: new Set((links.data ?? []).map((row: any) => row[relation.sourceId])),
      };
    },
  });
  const toggle = async (id: string, checked: boolean) => {
    const command = checked
      ? db.from(relation.link).insert({ doctor_id: doctorId, [relation.sourceId]: id })
      : db.from(relation.link).delete().eq("doctor_id", doctorId).eq(relation.sourceId, id);
    const { error } = await command;
    if (!error) void client.invalidateQueries({ queryKey: key });
  };
  return (
    <div>
      <h4 className="text-sm font-semibold">{relation.label}</h4>
      <div className="mt-2 max-h-44 overflow-y-auto rounded-md border border-border p-2">
        {query.data?.options.length ? (
          query.data.options.map((option: any) => (
            <label
              key={option.id}
              className="flex items-start gap-3 rounded p-2 text-sm hover:bg-secondary"
            >
              <Checkbox
                checked={query.data?.linked.has(option.id)}
                onCheckedChange={(value) => toggle(option.id, value === true)}
              />
              <span>{option[relation.text]}</span>
            </label>
          ))
        ) : (
          <p className="p-2 text-sm text-muted-foreground">No records available.</p>
        )}
      </div>
    </div>
  );
