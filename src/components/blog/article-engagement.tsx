  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 id="discussion-title" className="text-2xl font-semibold">
          Article discussion
        </h2>
        {engagementEnabled ? (
          <Button
          ...
          >
            <Heart className="size-4" aria-hidden="true" /> {liked ? "Liked" : "Like"} (
            {engagement.data?.likes.length ?? 0})
          </Button>
        ) : null}
      </div>
      {notice ? (
      ...
      {commentsEnabled ? (
        <>
          <div className="mt-6 grid gap-3">
            <label htmlFor="article-comment" className="font-medium">
              Add a comment
            </label>
            <Textarea
              id="article-comment"
            ...
            <Button
              ...
              disabled={!userId || submit.isPending}
            ...
            {engagement.data?.comments.map((item) => (
              <article key={item.id} className="border-l-2 border-primary pl-4">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  {item.profiles?.full_name ?? "Reader"} ·{" "}
