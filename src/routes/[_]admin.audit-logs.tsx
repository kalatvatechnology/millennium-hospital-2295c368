...
function AdminAuditLogs() {
  ...
  const filtered = useMemo(() => {
    ...
    return (logs.data ?? []).filter((row) => !term || `${row.actorEmail ?? ""} ${row.action} ${row.summary ?? ""} ${row.entityTable ?? ""}`.toLowerCase().includes(term));
  }, [logs.data, search]);
  ...
  const columns: Column<(typeof filtered)[number]>[] = [
    ...
    { key: "what", header: "Action", cell: (row) => row.summary ?? `${row.action} ${row.entityTable ?? ""}` },
  ];
