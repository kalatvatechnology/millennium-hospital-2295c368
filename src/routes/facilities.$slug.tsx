...
import { DepartmentCard, DoctorCard, HospitalServiceCard, ProfessionalServiceCard } from "@/components/content/cards";
    ...
    <PublicPage>
      <Async query={query}>
        {(data) =>
          !data ? (
            <ContentSection>
              <EmptyState title="We couldn't find this facility" description="It may not be published yet or the address may have changed." action={<Button asChild><Link to="/facilities">View all facilities</Link></Button>} />
            </ContentSection>
          ) : (
            <>
              <PageIntro eyebrow="Facility" title={data.facility.name} description={data.facility.description ?? "Further information has not yet been published."} />
              ...
              {data.professionalServices.length || data.hospitalServices.length ? (
                <ContentSection muted>
                  ...
                  <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {data.professionalServices.map((item) => <ProfessionalServiceCard key={item.id} service={item} />)}
                    {data.hospitalServices.map((item) => <HospitalServiceCard key={item.id} service={item} />)}
                  </div>
                </ContentSection>
              ) : null}
              <ContentSection>
                <div className="text-center"><Button asChild><Link to="/contact">Plan your visit</Link></Button></div>
              </ContentSection>
            </>
          )
        }
