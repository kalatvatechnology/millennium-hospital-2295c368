...
export const Route = createFileRoute("/_admin/blog-comments/$commentId")({
  head: () => ({
    meta: [
      ...createPageMeta("Review blog comment", "Moderate a reader comment."),
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error: updateError } = await supabase
        .from("blog_comments")
        .update({ status, moderated_by: session.user.id, moderated_at: new Date().toISOString() })
        .eq("id", commentId);
      if (updateError) throw updateError;
      await logAction({
        action: "blog_comment_moderated",
        entityTable: "blog_comments",
        entityId: commentId,
        summary: `Blog comment marked ${status}`,
      });
    },
    onSuccess: async () => {
      await client.invalidateQueries({ queryKey: ["admin-blog-comments"] });
      void navigate({ to: "/_admin/blog-comments" });
    },
    onError: (cause: Error) => setError(userFacingDataError(cause)),
  });
  return (
    <AdminShell
      title="Review blog comment"
      description={comment.data?.blog_posts?.title ?? "Reader comment"}
      requires="content.write"
    >
      <WorkspaceLayout backLink={<Link to="/_admin/blog-comments">All blog comments</Link>}>
        <WorkspaceSection
          title="Comment"
          description="Comment text is read-only. Choose whether it may appear publicly."
        >
          <AdminError message={error} />
          <div>
            <Label htmlFor="comment-author">Author</Label>
            <p id="comment-author" className="mt-2 text-sm">
              {comment.data?.profiles?.full_name ?? "Reader"}
            </p>
          </div>
          <div>
            <Label htmlFor="comment-content">Submitted comment</Label>
            <Textarea
              id="comment-content"
              className="mt-2"
              readOnly
          ...
          <div>
            <Label htmlFor="comment-status">Moderation status</Label>
            <Select value={status} onValueChange={(value) => setStatus(value as typeof status)}>
              <SelectTrigger id="comment-status" className="mt-2">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <StatusBadge
            status={status}
            tone={status === "approved" ? "positive" : status === "hidden" ? "critical" : "warning"}
          />
          <WorkspaceSaveBar
            busy={save.isPending}
            onCancel={() => navigate({ to: "/_admin/blog-comments" })}
            onSave={() => save.mutate()}
          />
        </WorkspaceSection>
      </WorkspaceLayout>
